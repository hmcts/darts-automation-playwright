...
Feature: End-to-end Approver
  ...
  Scenario Outline: Requester Approver
    ...
    When I load an audio file
      | courthouse   | courtroom   | case_numbers  | date       | startTime   | endTime   | audioFile   |
      | <courthouse> | <courtroom> | <case_number> | {{date+0}} | <startTime> | <endTime> | <audioFile> |
    Given I am logged on to DARTS as a "REQUESTERAPPROVER" user
    Then I set "Case ID" to "<case_number>"
    Then I press the "Search" button
    Then I see "1 result" on the page
    Then I see "<case_number>" in the same row as "<courthouse>"
    ...
    Examples:
      ...
      | Harrow Crown Court | {{seq}}-87 | S{{seq}}053-B | S{{seq}} judge-B087 | S{{seq}} defendants-B087 | S{{seq}} prosecutors-B | S{{seq}} defenders-B | {{displaydate}} | Sentencing remarks | Overnight | {{seq}}001 | {{seq}}001 |               |               | {{yyyymmdd hh:mm:ss}} | sample1.mp2 | 18:03:00  | 18:04:00 |
