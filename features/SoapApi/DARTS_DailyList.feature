...
Feature: Add Daily List using SOAP
  ...
  Scenario Outline: Daily List Single Case Scenario - lists for today and tomorrow
    ...
    And I see table "CASE_HEARING" column "case_closed" is "false" where "cas.cas_id" = "{{cas.cas_id}}"
  ...
  Scenario: Daily List successful with cases containing differing whitespace
    ...
    # Given I wait until there is not a daily list waiting for "YORK"
    Given I authenticate from the "CPP" source system
    And I see table "darts.daily_list" column "count(job_status)" is "0" where "unique_id" = "CSDDL170974{{seq}}003" and "message_id" = "58b211f5-426d-81be-00{{seq}}003"
    When I call POST SOAP API using soap action "addDocument" and body:
      """
      <messageId>58b211f5-426d-81be-00{{seq}}003</messageId>
      <type>DL</type>
      <subType>DL</subType>
      <document>
      <![CDATA[<cs:DailyList xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:p2="http://www.govtalk.gov.uk/people/bs7666" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails">
      <cs:DocumentID>
      <cs:DocumentName>DailyList_467_{{date+0}}000002.xml</cs:DocumentName>
      <cs:UniqueID>CSDDL170974{{seq}}003</cs:UniqueID>
      <cs:DocumentType>DL</cs:DocumentType>
      <cs:TimeStamp>{{timestamp}}</cs:TimeStamp>
      </cs:DocumentID>
      <cs:ListHeader>
      ...
      <cs:TimeMarkingNote>3:00 PM</cs:TimeMarkingNote>
      <cs:CaseNumber>  T{{seq}}121  </cs:CaseNumber>
      <cs:Prosecution ProsecutingAuthority="Other Prosecutor">
      <cs:ProsecutingReference>Prosecution ref</cs:ProsecutingReference>
      <cs:ProsecutingOrganisation>
      <cs:OrganisationName>Other Prosecutor</cs:OrganisationName>
      ...
      """
    Then the API status code is 200
    And I see table "darts.daily_list" column "job_status" is "NEW" where "unique_id" = "CSDDL170974{{seq}}003" and "message_id" = "58b211f5-426d-81be-00{{seq}}003"
    When I process the daily list for courthouse "YORK"
    And I wait for case "  T{{seq}}121  " courthouse "YORK"
    And I wait for case "T{{seq}}121  " courthouse "YORK"
    Then I see table "darts.daily_list" column "job_status" is "PROCESSED" where "unique_id" = "CSDDL170974{{seq}}003" and "message_id" = "58b211f5-426d-81be-00{{seq}}003"
    And I see table "CASE_HEARING" column "case_closed" is "false" where "case_number" = "  T{{seq}}121  " and "courthouse_name" = "YORK" and "courtroom_name" = "1"
    And I see table "CASE_HEARING" column "case_closed" is "false" where "case_number" = "T{{seq}}121  " and "courthouse_name" = "YORK" and "courtroom_name" = "1"
